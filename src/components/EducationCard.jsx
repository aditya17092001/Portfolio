...
const EducationCard = ({ year, institutionName, level, cgpa, percentage }) => {
    ...
    return ( 
            <div className='w-2/3 items-center px-5 py-3 bg-white/10 backdrop-filter backdrop-blur-lg shadow-lg rounded-full'>
                <div className='flex justify-center text-green-400 text-lg'>{year}</div>
                <div className='flex'>
                    <div className='h-14 w-14'>{educationlogo}</div>
                    <div className='h-16 border bg-white ml-3'></div>
                    <div className=''>
                        <div className='flex justify-start text-2xl ml-5 text-pink-600'>{level}</div>
                        <div className='flex justify-start text-xl ml-5'>{institutionName}</div>
                        <div className='flex justify-start ml-5'>{cgpa ? `CGPA: ${cgpa}/10.0` : `Percentage: ${percentage}/100.0`}</div>
                    </div>
                </div>
            </div>
    )
